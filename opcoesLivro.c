...
void emprestarLivro(rgLivro *lstLivro, int nLivro, rgUsuario *lstUsuario, int nUsuario) {
    ...
    if (posicaoLivro == -1) {
        ...
        if (posicaoUsuario == -1) {
            ...
            if (confirmou("Confirma o empréstimo? (S/*): ")) {
...
void importarLivros(rgLivro *livros, int *nLivros, rgUsuario *usuarios, int nUsuarios) {
    ...
    while (fscanf(arq, "%d;%99[^;];%99[^;];%d;%d;%c\n", 
        ...
        // Se o livro foi emprestado (idUsuario != -1), adicione ele à lista de livros emprestados do usuário
        if (livros[*nLivros - 1].idUsuario != -1) {
            int idUsuario = livros[*nLivros - 1].idUsuario;
            
            // Verifica se o idUsuario é válido
            if (idUsuario > 0 && idUsuario <= nUsuarios) {
                // Adiciona o livro na lista de livros emprestados do usuário
                for (int i = 0; i < 10; i++) {  // Supondo que cada usuário pode ter até 10 livros emprestados
